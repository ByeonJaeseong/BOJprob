...
class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    for (int i = 0; i < n; i++) {
      ...
      int q =1;
      while(Integer.parseInt(inputNo[q])!=-1){
      
      int x = Integer.parseInt(inputNo[q]) - 1;
      int w = Integer.parseInt(inputNo[q+1]);
      nodes[p].adj.add(nodes[x]);
      nodes[x].adj.add(nodes[p]);
      nodes[p].weights.add(w);
